...
/// </summary>
public class FluidSPHArticle : MonoBehaviour
{
...
"Higher values make the fluid effectively stiffer and reduce density error by allowing pressure waves to propagate faster.")]
    ...
    private ComputeBuffer invDensitiesBuffer;
    private ComputeBuffer pressureBuffer;
    private ComputeBuffer forcesBuffer;
    private ComputeBuffer xsphBuffer;


    private ComputeBuffer nbrStartBuffer;
    private ComputeBuffer nbrEndBuffer;
    private ComputeBuffer nbrFlatBuffer;

    private ComputeBuffer particleIndexBuffer;

    private ComputeShader cs;
    private int kernelDensity;
    private int kernelComputeForcesWithXSPH;
    private int kernelPressure;

    private int kernelIntegrate;

    private static readonly int RotationMatrixID = Shader.PropertyToID("_rotationMatrix");
    private static readonly int RotationMatrixInverseID = Shader.PropertyToID("_rotationMatrixInverse");
    ...
    void Awake()
    {
        ...
        positionsBuffer = new ComputeBuffer(numberOfParticles, sizeof(float) * 3);
        positionsBuffer.SetData(positions);
        velocityBuffer = new ComputeBuffer(numberOfParticles, sizeof(float) * 3);
        velocityBuffer.SetData(velocities);

        densitiesBuffer = new ComputeBuffer(numberOfParticles, sizeof(float)); 
        invDensitiesBuffer = new ComputeBuffer(numberOfParticles, sizeof(float)); 

        pressureBuffer = new ComputeBuffer(numberOfParticles,sizeof(float));
        forcesBuffer = new ComputeBuffer(numberOfParticles,sizeof(float)*3);
        xsphBuffer = new ComputeBuffer(numberOfParticles,sizeof(float)*3);

        nbrStartBuffer = new ComputeBuffer(numberOfParticles, sizeof(int));
        nbrEndBuffer = new ComputeBuffer(numberOfParticles, sizeof(int));
        nbrFlatBuffer = new ComputeBuffer(numberOfParticles * 64, sizeof(int));


        cs = Resources.Load<ComputeShader>("SPHCompute");

        
        kernelDensity = cs.FindKernel("ComputeDensity");
        kernelPressure = cs.FindKernel("ComputePressure");
        kernelComputeForcesWithXSPH = cs.FindKernel("ComputeForcesWithXSPH");
        kernelIntegrate = cs.FindKernel("Integrate");

        cs.SetBuffer(kernelDensity, "_positions", positionsBuffer);
        cs.SetBuffer(kernelDensity, "_densities", densitiesBuffer);
        cs.SetBuffer(kernelDensity, "_invDensities", invDensitiesBuffer);


        cs.SetBuffer(kernelPressure, "_densities", densitiesBuffer);
        cs.SetBuffer(kernelPressure,"_pressures",pressureBuffer);
        
        cs.SetBuffer(kernelComputeForcesWithXSPH, "_positions", positionsBuffer);
        cs.SetBuffer(kernelComputeForcesWithXSPH, "_velocities", velocityBuffer);
        cs.SetBuffer(kernelComputeForcesWithXSPH, "_densities", densitiesBuffer);
        cs.SetBuffer(kernelComputeForcesWithXSPH, "_invDensities", invDensitiesBuffer);
        cs.SetBuffer(kernelComputeForcesWithXSPH,"_pressures",pressureBuffer);
        cs.SetBuffer(kernelComputeForcesWithXSPH, "_forces", forcesBuffer);
        cs.SetBuffer(kernelComputeForcesWithXSPH, "_xsphDelta", xsphBuffer);

        cs.SetBuffer(kernelIntegrate, "_positions", positionsBuffer);
        cs.SetBuffer(kernelIntegrate, "_velocities", velocityBuffer);
        cs.SetBuffer(kernelIntegrate, "_densities", densitiesBuffer);
        cs.SetBuffer(kernelIntegrate, "_forces", forcesBuffer);
        cs.SetBuffer(kernelIntegrate, "_xsphDelta", xsphBuffer);
        
        cs.SetFloat("_h", h);
        cs.SetFloat("_h2", h2);
        cs.SetFloat("_mass", mass);
        cs.SetFloat("_wendlandCoeff", sphParams.wendlandC2Coeff);
        cs.SetFloat("_restDensity", restDensity);
        cs.SetFloat("_minRatio", minRatio);
        cs.SetFloat("_maxRatio", maxRatio);

        cs.SetInt("_particleCount", numberOfParticles);
        cs.SetInt("_eosType",(int) eosType);


        uint[] args = new uint[5];
        args[0] = sphereMesh.GetIndexCount(0);
        args[1] = (uint)numberOfParticles;
        ...
        Shader shader = Shader.Find("SPH/ParticleRender");
    ...
    Graphics.DrawMeshInstancedIndirect(sphereMesh, 0, material, bounds, argsBuffer);
    ...
    {   
        ...
        float3 [] positionsArray = new float3[numberOfParticles];
        positionsBuffer.GetData(positionsArray);
        positions.CopyFrom(positionsArray);
        ...
        handle.Complete();
        int sizeNbrFlat = grid.NbrEnd[numberOfParticles-1];
        nbrFlatBuffer.SetData(grid.NbrFlat, 0, 0, sizeNbrFlat);
        nbrStartBuffer.SetData(grid.NbrStart);
        nbrEndBuffer.SetData(grid.NbrEnd);
     ...
     private void singlePassWCSPH(float dt)
    {
        ...
        cs.SetFloat("_dt", dt);
        cs.SetFloat("_soundSpeed",soundSpeed);
        cs.SetFloat("_gamma",gamma);
        cs.SetFloat("_pressureStiff",pressureStiff);
        cs.SetBool("_densityClamping",densityClamping);

       

        cs.SetFloat("_viscosity",viscosity);
        cs.SetFloat("_avEps",avEps);
        cs.SetFloat("_avAlpha",avAlpha);
        cs.SetFloat("_avBeta",avBeta);

        // XSPH parameters
        cs.SetFloat("_xsphEps", xsphEps);
        cs.SetBool("_useXSPH", useXSph);
        cs.SetBool("_xsphAppliesToPosition", xsphAppliesToPosition);
        
        cs.SetFloat("_maxVelocity", maxVelocity);
        cs.SetBool("_damp", densityDamping);
        cs.SetFloat("_restDampingThresholdSpeed", speedThreshold);
        cs.SetFloat("_densityErrorThreshold", densityErrorThreshold);
        cs.SetFloat("_restDampingFactor", restDampingFactor);

        cs.SetFloat("_gravity",physicsParams.gravity.y);

        // Density clamping
        cs.SetBool("_densityClamping", densityClamping);

        // Collision parameters
        cs.SetVector("_boundsSize", new Vector3(boundsSize.x, boundsSize.y, boundsSize.z));
        cs.SetFloat("_collisionRadius", collisionRadius);
        cs.SetFloat("_collisionDamping", collisionDamping);
        cs.SetVector("_boxPosition", transform.position);

        //Box velocity (compute from previous frame)
        float3 deltaPos = (float3) transform.position - boxPrevPos;
        float3 boxVelocity = deltaPos / dt;
        cs.SetVector("_linearVelB", new Vector3(boxVelocity.x, boxVelocity.y, boxVelocity.z));

        // Box angular velocity
        Quaternion deltaRot = transform.rotation * Quaternion.Inverse(qPrevBox);
        deltaRot.ToAngleAxis(out float angleDeg, out Vector3 axis);
        float3 angularVelocity = (float3)axis * (angleDeg * Mathf.Deg2Rad / dt);
        cs.SetVector("_angularVelB", new Vector3(angularVelocity.x, angularVelocity.y, angularVelocity.z));

        // Rotation matrices
        Matrix4x4 rotMat = Matrix4x4.Rotate(transform.rotation);
        cs.SetMatrix(RotationMatrixID, rotMat);
        cs.SetMatrix(RotationMatrixInverseID, rotMat.inverse);
        ...
        cs.SetBuffer(kernelDensity, "_nbrFlat", nbrFlatBuffer);
        cs.SetBuffer(kernelDensity, "_nbrStart", nbrStartBuffer);
        cs.SetBuffer(kernelDensity, "_nbrEnd", nbrEndBuffer);

        cs.SetBuffer(kernelComputeForcesWithXSPH, "_nbrFlat", nbrFlatBuffer);
        cs.SetBuffer(kernelComputeForcesWithXSPH, "_nbrStart", nbrStartBuffer);
        cs.SetBuffer(kernelComputeForcesWithXSPH, "_nbrEnd", nbrEndBuffer);


        int threadGroupSize = 256;
        int threadGroups = Mathf.CeilToInt((float)numberOfParticles / threadGroupSize);

        cs.Dispatch(kernelDensity, threadGroups, 1, 1);
        cs.Dispatch(kernelPressure,threadGroups,1,1);
        cs.Dispatch(kernelComputeForcesWithXSPH,threadGroups,1,1);
        cs.Dispatch(kernelIntegrate,threadGroups,1,1);
    ...
    private void OnDestroy()
    {
        ...
        invDensitiesBuffer?.Release();
        xsphBuffer?.Release();
        forcesBuffer?.Release();
        velocityBuffer?.Release();
        nbrStartBuffer?.Release();
        nbrEndBuffer?.Release();
        nbrFlatBuffer?.Release();
    ...
    public static void ResolveAxis(float pen, float3 normal, int i, float deltaT, NativeArray<float3> positions
        , float3x3 rotationMatrix, NativeArray<float3> velocities, float collisionDamping, float maxSpeed)
    {
        if (pen < 0f)
        {
            float d = -pen;
            ...
            //Normal in world space
            float3 nW = math.mul(rotationMatrix, normal);

            float safetyMargin = 1.01f; 
            positions[i] += d * safetyMargin * nW;


            //Velocity reflection with damping
            float vn = math.dot(velocities[i], nW);
            if (vn < 0f)
            {
                float restitution = 1f - collisionDamping;
                float vnMag = math.abs(vn);

                velocities[i] -= (1f + restitution) * vn * nW;

            }
            else
            {
                velocities[i] -= math.min(0f, vn) * nW;
            }
